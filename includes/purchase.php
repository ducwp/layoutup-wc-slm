...
function wc_slm_create_license_keys($order_id)
{
    ...
    foreach ($items as $item => $values) {
        ...
        if(isset($values['variation_id']) && $values['variation_id'] !== 0 ) {
          $download_id = $product_id = $values['variation_id'];
        ...
        if (wc_slm_is_licensing_enabled($product_id)) {
            ...
            if ($product->is_downloadable()) {
                ...
                for ($i = 1; $i <= $download_quantity; $i++) {
                    ...
                    // License Key Prefix
                    $lic_prefix = wc_slm_get_lic_prefix($product_id);

                    // License Key Prefix
                    $item_ref = wc_slm_get_item_ref($product_id);
                    ...
                    $api_params = array();
                    $api_params['lic_prefix'] = $lic_prefix;
                    ...
                    $api_params['product_ref'] = $item_ref;
                    /**
 ...
 * @since  1.0.0
 */
function wc_slm_get_lic_prefix($product_id)
{

    $wc_slm_lic_prefix = get_post_meta($product_id, '_wc_slm_lic_prefix', true);

    if (empty($wc_slm_lic_prefix)) {
        return '';
    }

    return $wc_slm_lic_prefix;
}
 ...
 * @since  1.0.0
 */
function wc_slm_get_item_ref($product_id)
{

    $wc_slm_item_ref = get_post_meta($product_id, '_wc_slm_item_ref', true);

    if (empty($wc_slm_item_ref)) {
        return '';
    }

    return $wc_slm_item_ref;
}
