...
function wc_slm_custom_general_fields()
{
    ...
    $wc_slm_lic_prefix = esc_attr(get_post_meta($post_id, '_wc_slm_lic_prefix', true));
    $wc_slm_item_ref = esc_attr(get_post_meta($post_id, '_wc_slm_item_ref', true));
    ...
    <div class="options_group show_if_downloadable">
        ...
        <div <?php echo $wc_slm_display; ?> class="wc-slm-toggled-hide">
            ...
            <p class="form-field">
                <label for="_wc_slm_lic_prefix"><?php _e('License Key Prefix', 'wc-slm'); ?></label>
                <input type="text" name="_wc_slm_lic_prefix" class="small-text"
                       value="<?php echo $wc_slm_lic_prefix; ?>"/>
                <span class="woocommerce-help-tip"
                      data-tip="<?php _e('You can optionaly specify a prefix for the license keys. This prefix will be added to the uniquely generated license keys', 'wc-slm'); ?>"></span>
            </p>

            <p class="form-field">
                <label for="_wc_slm_item_ref"><?php _e('Item Reference Slug', 'wc-slm'); ?></label>
                <input type="text" name="_wc_slm_item_ref" class="small-text"
                       value="<?php echo $wc_slm_item_ref; ?>"/>
                <span class="woocommerce-help-tip"
                      data-tip="<?php _e('Enter slug for item reference slug. Eg. my-wp-plugin', 'wc-slm'); ?>"></span>
            </p>
        </div>
    </div>
    <style>
    .layoutup_checkbox label{
      margin-left: 4px;
    }
    </style>
    <?php
...
function wc_slm_custom_general_fields_save($post_id)
{
    ...
    update_post_meta(
        $post_id,
        '_wc_slm_lic_prefix',
        stripslashes($_wc_slm_lic_prefix)
    );

    update_post_meta(
        $post_id,
        '_wc_slm_item_ref',
        stripslashes($_wc_slm_item_ref)
    );
